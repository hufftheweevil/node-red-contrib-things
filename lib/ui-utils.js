if (!THI) {
  ...
  const primitiveTypes = ['str', 'num', 'bool', 'json']
  ...
  THI.makeLabel = thing => {
    ...
    if (thing.disabled) label += ' (disabled)'
    return label
  }
  ...
  THI.addState = (row, i, data) => {
    ...
    if (data.hasOwnProperty('value')) {
      THI.valueToTypedInput(value, data.value)
    } else {
    ...
    list.editableList('items').each(function () {
      ...
      if (val.type != 'proxy') {
        ...
        if (proxyFrom.type == 'single') {
          ...
          if (s.fn == 'custom')
            s.fn = $(this).find('.node-input-list-function').data().editor.getValue()
        }
        // Key usage
  ...
  THI.addCommand = (row, i, data) => {
    ...
    let cmd = $('<input/>', { class: 'node-input-list-cmd' })
      ...
      .on('change', function (e, type) {
        if (lastCmdType == 'bool' && type != 'bool')
          setTimeout(() => $(this).typedInput('value', ''), 0)
        lastCmdType = type
        let asTypes = type == 'key' ? [KEY_TYPE] : primitiveTypes
        as.typedInput('types', asTypes)
      })
    ...
    let as = $('<input/>', { class: 'node-input-list-as' })
      .appendTo(row2)
      .typedInput({ types: primitiveTypes })
      .typedInput('hide')
    ...
    if (data.hasOwnProperty('as')) {
      ...
      THI.valueToTypedInput(as, data.as)
    }
  }
  ...
  THI.saveCommands = list => {
    ...
    list.editableList('items').each(function () {
      ...
      if (transform) c.as = getTypedInput($(this).find('.node-input-list-as'), { realVal: true })
  ...
  THI.valueToTypedInput = (input, value) => {
    setTypedInput(input, getType(value), typedToString(value))
  }
