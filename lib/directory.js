let ThingsDirectory = (function () {
  ...
  function makeThingRow(thing) {
    EXPANDED_STATE[thing.name] = EXPANDED_STATE[thing.name] || new Set()
    EXPANDED_PROPS[thing.name] = EXPANDED_PROPS[thing.name] || new Set()
            ...
            .dialog({
              ...
              open: function () {
                $(this)
                  ...
                  .typedInput({
                    types: [
                      ...
                      'bool',
                      {
                        value: 'json', // Need to use custom type here to hide expansion option
                        label: 'JSON',
                ...
                $(this)
                  ...
                  .on('keyup', function (e) {
                    if (e.key === 'Enter' || e.keyCode === 13) $('#things-dir-send-command').click()
                  })
              },
              title: `Send command to ${thing.name}`,
              buttons: [
                {
                  id: 'things-dir-send-command',
                  text: 'Send',
                  class: 'primary',
                  click: function () {
                    let command = THI.getTypedInput($(this).find('input'), { realVal: true })
                    socket.send(
                      JSON.stringify({
                        topic: 'send-command',
                        payload: {
                ...
                $('<div id="red-ui-confirm-dialog" class="hide">')
                  ...
                  .dialog({
                    ...
                    open: function () {
                      ...
                      THI.valueToTypedInput(ti, value)
                      $(this)
                        .find('.red-ui-typedInput-input')
                        .on('keyup', function (e) {
                          if (e.key === 'Enter' || e.keyCode === 13)
                            $('#things-dir-update-state').click()
                        })
                    },
                    title: `Update state "${key}" for ${thing.name}`,
                    buttons: [
                      {
                        id: 'things-dir-update-state',
                        text: 'Update',
                        class: 'primary',
                        click: function () {
                          let value = THI.getTypedInput($(this).find('input'), { realVal: true })
                          socket.send(
                            JSON.stringify({
                              topic: 'update-state',
                              payload: {
        ...
        RED.utils
          .createObjectElement(value, {
            ...
            ontoggle: (path, expanded) =>
              EXPANDED_STATE[thing.name][expanded ? 'add' : 'delete'](path.slice(6) + '.'),
            expandPaths: Array.from(EXPANDED_STATE[thing.name])
          })
          .appendTo(stateCol)
          .find('.red-ui-debug-msg-type-number-toggle')
          .filter(function () {
            return /^\d+$/.test($(this).html())
          })
          .click()
          .click()
      })
      ...
      props.forEach(([key, value]) =>
        RED.utils
          .createObjectElement(value, {
            ...
            rootPath: 'props',
            ontoggle: (path, expanded) =>
              EXPANDED_PROPS[thing.name][expanded ? 'add' : 'delete'](path.slice(6) + '.'),
            expandPaths: Array.from(EXPANDED_PROPS[thing.name])
          })
          .appendTo(propsCol)
          .find('.red-ui-debug-msg-type-number-toggle')
          .filter(function () {
